...
impl User {
    ...
    pub async fn create_user_tables(pool: &SqlitePool) {
        ...
        sqlx::query(
            r#"
                INSERT INTO users
                    (id, anonymous, username, pwhash) VALUES ($1, $2, $3, $4)
        ...
        let hashed = bcrypt::hash("welcome").unwrap();
...
#[derive(Debug)]
pub struct Session(
    pub axum_session_auth::AuthSession<crate::auth::User, i64, SessionSqlitePool, sqlx::SqlitePool>,
);
